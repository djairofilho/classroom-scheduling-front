  ...
  return (
    <>
      <PageIntro
        ...
        actions={<Button>Nova reserva</Button>}
      />
            ...
            <tbody>
              {filteredBookings.map((booking) => (
                <tr key={booking.id} className="border-t border-stroke transition hover:bg-brand-paper">
                  <td className="px-6 py-5 text-sm font-semibold text-ink">{booking.id}</td>
                  ...
                  <td className="px-6 py-5">
                    <div className="flex justify-end gap-3 text-sm font-semibold">
                      ...
                      {booking.status === 'Ativa' ? (
                        <button className="text-brand-red" onClick={() => setShowCancelModal(true)} type="button">
                          Cancelar
                        </button>
                      ) : null}
                    </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="mt-3 text-sm leading-7 text-ink-muted">
                  Tem certeza que deseja cancelar a reserva <strong>#RES-001</strong> para o{' '}
                  <strong>Laboratorio de Informatica 3</strong>? Esta acao nao pode ser desfeita e o espaco sera liberado imediatamente.
                </p>
              </div>
            </div>
            <div className="mt-6 flex justify-end gap-3 border-t border-stroke pt-5">
              <Button tone="secondary" onClick={() => setShowCancelModal(false)}>
                Voltar
              </Button>
              <Button onClick={() => setShowCancelModal(false)}>Confirmar cancelamento</Button>
            </div>
          </Card>
        </div>
      ) : null}
